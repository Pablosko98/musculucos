...
import { View, FlatList, TouchableOpacity, TextInput, ScrollView, Animated } from 'react-native';
import { Text } from '@/components/ui/text';
import { Search, Star, X, ChevronRight } from 'lucide-react-native';
import { Exercise, MUSCLE_GROUP_MAP, HEAD_LABELS } from '@/lib/exercises';
import type { ExerciseStat } from '@/lib/db';
import { EQUIPMENT_COLORS, fmtEquipment, variantLabel, fmt } from './analyticsUtils';
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <ExerciseGroupCard group={item} stats={stats} onSelectVariant={onSelectVariant} />
        )}
        ListHeaderComponent={
          <Text style={{ color: '#52525b', fontSize: 13, marginBottom: 10 }}>
            {loading
              ? 'Loading…'
              : `${filteredGroups.length} exercise${filteredGroups.length !== 1 ? 's' : ''}`}
          </Text>
        }
        ListEmptyComponent={
          loading ? null : (
